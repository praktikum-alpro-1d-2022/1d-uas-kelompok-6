#include<iostream>
#include <iomanip> //membuat tabel
#include <Windows.h> //khusus untuk windows, berfungsi mengatur waktu delay ke perintah selanjutnya
using namespace std;
struct detail{
        string drname;
...
void showallmenu(detail menu[]){//menampilkan semua menu
int s,awal;
char next;
system("CLS");
habis:
...
void select(){//pemilihan menu oleh user
char interes,next;
int slc,banyak;
cout<<"Ada menu yang Anda pilih? (y/n) : ";
 ...
 if (interes=='y'){
    ...
    do{
            ...
            cin>>slc;
            if (slc>0 &&slc<31){
            order[i].kode=slc;}else{cout<<"kode pesanan invalid"<<endl;goto failkode;}
            failbanyak:
            cout<<"Masukkan jumlah \t\t: ";
            cin>>banyak;
            if(banyak>0){
            order[i].jumlah=banyak;}else{cout<<"Jumlah harus lebih dari 0"<<endl;goto failbanyak;}
            cout<<"Ada lagi? \n...ya (y) \n...menu selanjutnya (n) \t: ";
            cin>>next;
            i++;
            }
...
void display(detail menu[],int i){//menampilkan menu di function rekomendasi
  cout<<"| "<<setiosflags(ios::left)<<setw(5)<<i+1<<"|";
  cout<<" "<<setiosflags(ios::left)<<setw(40)<<menu[i].drname<<"|";
  cout<<" "<<setiosflags(ios::left)<<setw(40)<<menu[i].price<<"|";
  cout<<"\n============================================================================================\n";
 }
void recomend(detail menu[]){//rekomendasi
    system("CLS");
    int price;
...
void struk (detail menu[]){//struk akhir
    system("CLS");
   cout<<"=================================================================================================\n";
...
void edit(detail menu[]){//untuk mengedit menu
basket* ubah[100];
    char slcedit;
...
void final(int tmpsum){//total semua menu
    float diskon,final;
